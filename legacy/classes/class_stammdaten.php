...
class stammdaten
{
    function pdf_stamm_objekt($pdf, $objekt_id)
    {
        ...
        $pdf->ezTable($pdf_tab, $cols, "Stammdaten Objekt $o->objekt_kurzname", array(
            'showHeadings' => 0,
            'shaded' => 1,
            'titleFontSize' => 9,
            'fontSize' => 8,
        ...
        if (!empty($gk_ids_arr)) {
            ...
            $pdf->ezTable($pdf_gk, $cols, "Geldkonten Objekt", array(
                'showHeadings' => 1,
                'shaded' => 1,
                'titleFontSize' => 8,
                'fontSize' => 8,
    ...
    function pdf_stamm_haus($pdf, $haus_id)
    {
        ...
        if ($anz_details) {
            ...
            $pdf->ezSetDy(-10); // abstand
            $pdf->ezTable($pdf_tab, $cols, "Details vom Haus $h->haus_strasse $h->haus_nummer, $h->haus_plz $h->haus_stadt", array(
                'showHeadings' => 0,
                'shaded' => 1,
                'titleFontSize' => 9,
                'fontSize' => 8,
    ...
    function stamm_einheiten_objekt(Cezpdf $pdf, $objekt_id)
    {
        $pdf->ezNewPage();
        $druckdatum = date("d.m.Y");
        $pdf->addText(464, 730, 7, "<b>Druckdatum: $druckdatum</b>");
        ...
        $pdf->ezSetDy(-10); // abstand
        $pdf->ezTable($einheit_arr, $cols, "Einheitenliste vom Objekt $o->objekt_kurzname", array(
            'showHeadings' => 1,
            'shaded' => 1,
            'titleFontSize' => 10,
            'fontSize' => 8,
        ...
        for ($a = 0; $a < $anz_einheiten; $a++) {
            ...
            if ($anz_details) {
                ...
                $pdf->ezTable($pdf_tab, $cols, "Details zu Einheit $e->einheit_kurzname", array(
                    'showHeadings' => 0,
                    'shaded' => 1,
                    'titleFontSize' => 8,
                    'fontSize' => 8,
                    ...
                    for ($pp = 0; $pp < $weg->anz_personen; $pp++) {
                        ...
                        if ($anz_details) {
                            ...
                            if (is_array($pdf_tabp)) {
                                ...
                                $pdf->ezSetDy(-5); // abstand
                                $pdf->ezTable($pdf_tabp, $cols, "Details zu Person <b>$pe->person_nachname $pe->person_vorname</b>", array(
                                    'showHeadings' => 0,
                                    'shaded' => 1,
                                    'titleFontSize' => 8,
                                    'fontSize' => 8,
            ...
            if (!empty($mv_ids)) {
                ...
                for ($m = 0; $m < $anz_mv; $m++) {
                    ...
                    $pdf->ezTable($pdf_mv, $cols, "Mietvertragsdaten $mv->einheit_kurzname | $mv->personen_name_string</b>", array(
                        'showHeadings' => 0,
                        'shaded' => 1,
                        'titleFontSize' => 8,
                        'fontSize' => 8,
