...
class geldkonto_info
{
    ...
    function alle_geldkonten_tabelle_kontostand()
    {
        ...
        if ($numrows > 0) {
            ...
            echo "<tr><th>Konto</th><th>Bezeichnung</th><th>Begünstigter</th><th width=\"220\">IBAN</th><th>BIC</th><th>Kontostand</th><th>Option</th><th>Option</th></tr>";
            for ($a = 0; $a < $numrows; $a++) {
                $zaehler++;
...
FROM `GELD_KONTO_BUCHUNGEN` WHERE GELDKONTO_ID = '$geld_konto_id' && AKTUELL = '1'");
        ...
        if ($numrows > 0) {
            ...
            for ($a = 0; $a < $numrows; $a++) {
                ...
                if (session()->has('geldkonto_id') && session()->get('geldkonto_id') == $konto_id) {
                    echo "<option value=\"$konto_id\" selected>$bezeichnung - $vwzk - Knr:$kontonummer - Blz: $blz</option>\n";
                } else {
                    echo "<option value=\"$konto_id\" >$bezeichnung - $vwzk - Knr:$kontonummer - Blz: $blz</option>\n";
                }
            } // end for
    ...
    function geld_konto_ermitteln($kostentraeger_typ, $kostentraeger_id, $zeitpunkt = null, $verwendungszweck = '')
    {
        if (is_null($zeitpunkt)) {
            $zeitpunkt = \Carbon\Carbon::now()->toDateString();
        }
        ...
        if ($geldkonten_anzahl) {
            ...
            $result = DB::select(
                "SELECT GELD_KONTEN.KONTO_ID, 
                    GELD_KONTEN.BEGUENSTIGTER, 
                    GELD_KONTEN.KONTONUMMER, 
                    GELD_KONTEN.BLZ, 
                    GELD_KONTEN.BEZEICHNUNG, 
                    GELD_KONTEN.INSTITUT, 
                    GELD_KONTEN.IBAN, 
                    GELD_KONTEN.BIC 
                FROM GELD_KONTEN_ZUWEISUNG, GELD_KONTEN 
                WHERE KOSTENTRAEGER_TYP = '$kostentraeger_typ' 
                    && KOSTENTRAEGER_ID = '$kostentraeger_id' 
                    && GELD_KONTEN.KONTO_ID = GELD_KONTEN_ZUWEISUNG.KONTO_ID 
                    && GELD_KONTEN_ZUWEISUNG.AKTUELL = '1' 
                    && GELD_KONTEN.AKTUELL = '1'
                    && GELD_KONTEN_ZUWEISUNG.VON <= '$zeitpunkt'
                    && (
                        GELD_KONTEN_ZUWEISUNG.BIS >= '$zeitpunkt'
                        || GELD_KONTEN_ZUWEISUNG.BIS IS NULL
                    )
                    && GELD_KONTEN_ZUWEISUNG.VERWENDUNGSZWECK = '$verwendungszweck'
                ORDER BY GELD_KONTEN.KONTO_ID ASC LIMIT 0,1"
            );
    ...
    function dropdown_geldkonten($kostentraeger_typ, $kostentraeger_id)
    {
        ...
        if ($numrows > 0) {
            ...
            for ($a = 0; $a < $numrows; $a++) {
                ...
                $vwzk = $my_array [$a] ['VERWENDUNGSZWECK'];
                if (session()->has('geldkonto_id') && session()->get('geldkonto_id') == $konto_id) {
                    echo "<option value=\"$konto_id\" selected>$bezeichnung - $vwzk - $geld_institut - Knr:$kontonummer - Blz: $blz</option>\n";
                } else {
                    echo "<option value=\"$konto_id\">$bezeichnung - $vwzk - $geld_institut - Knr:$kontonummer - Blz: $blz</option>\n";
                }
            }
            echo "</select>";
        } else {
